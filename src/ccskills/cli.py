"""ccskills CLI entry point."""

import argparse
from pathlib import Path

from ccskills.installer import install, uninstall


def cmd_install(args: argparse.Namespace) -> None:
    cwd = Path(args.cwd) if args.cwd else Path.cwd()
    skills = install(cwd)
    for name in skills:
        print(f"  installed: {name}")
    print(f"\n{len(skills)} skill(s) installed to {cwd / '.claude' / 'skills'}")


def cmd_uninstall(args: argparse.Namespace) -> None:
    cwd = Path(args.cwd) if args.cwd else Path.cwd()
    results = uninstall(cwd)
    for name, status in results.items():
        print(f"  {status}: {name}")
    removed = sum(1 for s in results.values() if s == "removed")
    skipped = sum(1 for s in results.values() if s == "skipped")
    print(f"\n{removed} skill(s) removed, {skipped} skipped")


def main() -> None:
    parser = argparse.ArgumentParser(
        prog="ccskills",
        description="Install Claude Code skills into your project",
    )
    parser.add_argument(
        "--cwd",
        metavar="DIR",
        help="target project directory (default: current directory)",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    install_parser = subparsers.add_parser("install", help="Install all skills")
    install_parser.set_defaults(func=cmd_install)

    uninstall_parser = subparsers.add_parser("uninstall", help="Uninstall all skills")
    uninstall_parser.set_defaults(func=cmd_uninstall)

    args = parser.parse_args()
    args.func(args)


if __name__ == "__main__":
    main()
